package com.allaboutscala.chapter.four.seven.caseclass

// we will learn how to define case class and use it to create instances of objects.

/*

A case class is similar to any other classes except that it also creates the Companion Object.
In addition, a case class will automatically create the apply(),  toString(), hashCode and equals()
methods for you.
...
object CaseClass extends App {
  ...
  let's create a Map to represent the type and quantity of donuts being bought by our customers.
  If you do not implement the hashCode method, adding and retrieving elements from the Map will give
  you unpredictable behaviour.

  However, since we are using the case class to represent the Donut objects, the hashCode method is
  implemented for us!
  ...
  // A case class also comes with a handy copy() method which you can use to copy an object and override
  // any field. In return, the copy() method will create new instances of the given object.
