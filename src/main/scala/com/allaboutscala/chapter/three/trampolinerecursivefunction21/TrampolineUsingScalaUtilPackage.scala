package com.allaboutscala.chapter.three.trampolinerecursivefunction21

// we will learn how to create trampoline tail recursive function by making use of utilities that
// Scala provides for tail recursions in the package scala.util.control.TailCalls._

// Say you had two tail recursive functions F(A) and F(B) and that F(A) calls F(B) but in turn F(B) also calls F(A).

// Then F(A) is said to be a trampoline tail recursive function because the call stack jumps back and forth
// between the two functions F(A) and F(B) - hence the name trampoline.

// tail recursive function will help prevent overflow in your call stack because the evaluation of your
// looping construct happens at each step.

import scala.util.control.TailCalls._

object TrampolineUsingScalaUtilPackage extends App {
  ...
  // tailcall(notSweetDonut(donutList)) defined in Step 2 below.

  /*
  It will then pass back the tail of the donut list to the verySweetDonut() function
  tailcall(verySweetDonut(donutList.tail))

  But if you recall from Step 1 above, the function verySweetDonut() will check the head of the donut
  list and if it does not find a sweet donut, it will call back the notSweetDonut() function.
